"""
Política de edición por oficio (PR4b): un oficio es editable si no está bloqueado por ruta o estado operativo.
"""
...
def iniciador_en_ruta_activa(ini: IniciadorRuta | None) -> bool:
    """
    Indica si el iniciador tiene un ``RutaItem`` activo en ruta operativa (BORRADOR/PUBLICADA/EN_CURSO).

    Parámetros:
        ini: iniciador o ``None``.

    Retorno:
        True si hay ítem no borrado en ruta con estado operativo activo.
    """
    if ini is None:
        return False
    return (
        ...
        .filter(
            ...
            RutaItem.deleted_at.is_(None),
            RutaTrabajo.estado_ruta.in_(("BORRADOR", "PUBLICADA", "EN_CURSO")),
        )
        .limit(1)
        .first()
        is not None
...
def evaluar_editable_oficio(oficio_id: int) -> dict[str, Any]:
    ...
    Retorno:
        Dict con ``editable``, ``bloqueado_motivo``, ``iniciador_id``, ``iniciador_estado``,
        ``ruta_item_id``, ``ruta_estado``, ``estado_ejecucion`` (opcionales).
    ...
    if ini is not None:
        ...
        if ri is not None:
            ...
            if ri.estado_ejecucion == "REALIZADO":
...
def existe_iniciador_en_ruta_activa_para_actuacion(actuacion_id: int) -> bool:
    """Subconsulta usada por bandejas: ¿algún REINSPECCION_OFICIO de la actuación está en ruta activa?"""

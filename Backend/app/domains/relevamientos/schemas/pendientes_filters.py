...
class RelevamientosPendientesFilters(BaseModel):
    """
    Schema para validar filtros de pendientes de Relevamientos.
    ...
    def validate_tipo(cls, v: Optional[str]) -> Optional[str]:
        ...
        if value not in {"domicilios"}:
            raise ValueError("tipo inválido.")
        return value

    @model_validator(mode="after")
    def apply_defaults_and_validate_range(self) -> "RelevamientosPendientesFilters":
        """
        Defaults:

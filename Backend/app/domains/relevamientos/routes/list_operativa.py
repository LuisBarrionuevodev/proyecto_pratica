...
@relevamiento.get("/gestion-operativa")
def listar_relevamientos_gestion_operativa():
    """
    Lista relevamientos operativos (solo con iniciador RELEVAMIENTO pendiente).
    """
    try:
        ...
        items_dto = [
            relevamiento_operativo_to_row(rel, iniciador.id, iniciador.estado_iniciador)
            for rel, iniciador in result["items"]
        ]

#!/usr/bin/env python
"""
PREDEPLOY-CLEANUP.3F.3 — backup + apply FASE 2C.2A (155 iniciador_ruta).
"""
...
from app.domains.predeploy_cleanup.apply_phase2c2a_service import (
    EXPECTED_EXEC_HASH,
    EXPECTED_PROT_HASH,
    EXPECTED_SOURCE_DIAG_HASH,
    apply_phase2c2a_initiators_cleanup,
    preflight_phase2c2a_apply,
)
...
EXEC_PATH = BACKEND_ROOT / "scripts" / "output" / "cleanup_execution_manifest_phase2c2a_initiators_20260920.json"
...
VERIFY_DB = "digitaliza_sandbox_backup_verify_phase2c2a"
...
def main() -> None:
    ...
    if not EXEC_PATH.is_file():
        raise SystemExit(f"Manifest no encontrado: {EXEC_PATH}")
    ...
    backup_path = BACKUP_DIR / f"digitaliza_sandbox_pre_cleanup_phase2c2a_{ts}.sql"
    dump_cmd = [
    ...
    with engine.connect() as conn:
        ...
        try:
            apply_report = apply_phase2c2a_initiators_cleanup(
                conn,
                execution_manifest,
                protected_manifest,
                SOURCE_DIAG_PATH,
                ...
                backup_exit_code=proc.returncode,
                restore_verified=True,
            ...
            out = OUTPUT_DIR / f"cleanup_phase2c2a_initiators_apply_{ts}.json"
        ...
        json.dumps(
            {
                ...
                "iniciador_ruta_after": apply_report["counts_after"]["iniciador_ruta"],
                "acts_unlock": apply_report["unlock"]["actuaciones"]["unlocked_count"],
                "rel_unlock": apply_report["unlock"]["relevamientos"]["unlocked_count"],
                "users_fk_free": apply_report["unlock"]["users"]["users_test_fk_free"],
                "report": str(out),
            },
            indent=2,
        )

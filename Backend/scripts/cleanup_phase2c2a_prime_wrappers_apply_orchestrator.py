#!/usr/bin/env python
"""
PREDEPLOY-CLEANUP.3H.2 — backup + apply FASE 2C.2A' (12 ruta_item + 38 iniciador_ruta).
"""
...
from app.domains.predeploy_cleanup.apply_phase2c2a_prime_service import (
    EXPECTED_EXEC_HASH,
    EXPECTED_PROT_HASH,
    EXPECTED_SOURCE_DIAG_HASH,
    apply_phase2c2a_prime_wrappers_cleanup,
    preflight_phase2c2a_prime_apply,
)
...
EXEC_PATH = (
    ...
    / "scripts"
    / "output"
    / "cleanup_execution_manifest_phase2c2a_prime_wrappers_20260920.json"
)
SOURCE_DIAG_PATH = (
    BACKEND_ROOT
    / "scripts"
    / "output"
    / "cleanup_phase2c2a_prime_residual_acts_diag_20260920.json"
)
...
VERIFY_DB = "digitaliza_sandbox_backup_verify_phase2c2a_prime"
...
def main() -> None:
    ...
    for p in (EXEC_PATH, SOURCE_DIAG_PATH, PROTECTED_PATH):
        if not p.is_file():
    ...
    source_diag_hash = file_sha256(SOURCE_DIAG_PATH)
    ...
    backup_path = BACKUP_DIR / f"digitaliza_sandbox_pre_cleanup_phase2c2a_prime_{ts}.sql"
    dump_cmd = [
    ...
    with engine.connect() as conn:
        ...
        try:
            apply_report = apply_phase2c2a_prime_wrappers_cleanup(
                conn,
                execution_manifest,
                protected_manifest,
                confirm_database=DB_NAME,
            ...
            out = OUTPUT_DIR / f"cleanup_phase2c2a_prime_wrappers_apply_{ts}.json"
        ...
        json.dumps(
            {
                ...
                "set_null_affected": apply_report["set_null_affected"],
                "counts_after": {
                    k: apply_report["counts_after"][k]
                    for k in ("ruta_item", "iniciador_ruta", "actuaciones", "orden_trabajo")
                },
                "acts_unlock": apply_report["future_2c2b_prime"]["unlocked_acts"]["UNLOCKED"],
                "users_fk_free": apply_report["users_unlock"]["users_test_fk_free_after_prime"],
                "report": str(out),
            },
            indent=2,
        )

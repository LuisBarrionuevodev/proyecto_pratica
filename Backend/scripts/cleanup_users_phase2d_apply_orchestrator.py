#!/usr/bin/env python
"""
PREDEPLOY-CLEANUP.3L.2 — backup + apply FASE 2D USERS (833 CONFIRMADO_TEST_FK_FREE).
"""
...
from app.domains.predeploy_cleanup.apply_users_phase2d_service import (
    EXPECTED_EXEC_HASH,
    EXPECTED_IDENTITY_HASH,
    EXPECTED_PROT_HASH,
    EXPECTED_SOURCE_DIAG_HASH,
    apply_users_phase2d_cleanup,
    preflight_users_phase2d_apply,
)
...
EXEC_PATH = BACKEND_ROOT / "scripts" / "output" / "cleanup_execution_manifest_users_phase2d_20260920.json"
...
VERIFY_DB = "digitaliza_sandbox_backup_verify_users_phase2d"
...
def run_smoke_qa() -> dict:
    ...
    with app.app_context():
        ...
        results["auth_user_note"] = "preserved system admin (user 1), not deleted SAFE user"
...
def main() -> None:
    ...
    for p in (EXEC_PATH, PROTECTED_PATH):
        if not p.is_file():
    ...
    if execution_manifest.get("source_diag_sha256") != EXPECTED_SOURCE_DIAG_HASH:
        raise SystemExit("ABORT: source_diag_sha256 mismatch")
    if execution_manifest.get("identity_snapshot_hash") != EXPECTED_IDENTITY_HASH:
        raise SystemExit("ABORT: identity_snapshot_hash mismatch")
    ...
    backup_path = BACKUP_DIR / f"digitaliza_sandbox_pre_cleanup_users_phase2d_{ts}.sql"
    dump_cmd = [
    ...
    safe_ids = set(execution_manifest["entities"]["users"])
    verify_counts = {
        "users": 2803,
        "profiles": 7,
    ...
    with create_engine(verify_uri).connect() as vconn:
        ...
        safe_found = 0
        for chunk_start in range(0, len(safe_ids), 400):
            chunk = sorted(safe_ids)[chunk_start : chunk_start + 400]
            ph = ",".join(str(i) for i in chunk)
            safe_found += vconn.execute(text(f"SELECT COUNT(*) FROM users WHERE id IN ({ph})")).scalar() or 0
        if safe_found != EXPECTED_SAFE:
            raise SystemExit(f"ABORT verify safe ids: {safe_found}")
        preserve_found = 0
        ph = ",".join(str(i) for i in sorted(PRESERVE_USER_IDS))
        preserve_found = vconn.execute(text(f"SELECT COUNT(*) FROM users WHERE id IN ({ph})")).scalar() or 0
        if preserve_found != len(PRESERVE_USER_IDS):
            raise SystemExit(f"ABORT verify preserve: {preserve_found}")
    ...
    with engine.connect() as conn:
        ...
        print("PHASE2D_PREFLIGHT_OK")

        try:
            apply_report = apply_users_phase2d_cleanup(
                conn,
                execution_manifest,
                protected_manifest,
            ...
            out = OUTPUT_DIR / f"cleanup_users_phase2d_apply_{ts}.json"
            out.write_text(
                json.dumps(
                    {
    ...
    apply_report["preflight_summary"] = {"status": "PHASE2D_PREFLIGHT_OK"}
    ...
    apply_report["manifest_hashes"] = {
        ...
        "identity_snapshot": EXPECTED_IDENTITY_HASH,
        ...
        json.dumps(
            {
                ...
                "users_after": apply_report["counts_after"]["users"],
                "fk_free_after": apply_report["fk_free_blocked_after"]["fk_free_count"],
                "report": str(out),
            },
            indent=2,
        )

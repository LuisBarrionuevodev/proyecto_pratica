#!/usr/bin/env python
"""
PREDEPLOY-CLEANUP.3E.2 — backup + apply FASE 2C.1 (actuaciones + denuncias + OT).
"""
...
from app.domains.predeploy_cleanup.apply_phase2c1_service import (
    apply_phase2c1_sources_cleanup,
    preflight_phase2c1_apply,
)
...
EXEC_PATH = BACKEND_ROOT / "scripts" / "output" / "cleanup_execution_manifest_phase2c1_sources_20260920.json"
...
DIAG_PATH = BACKEND_ROOT / "scripts" / "output" / "cleanup_phase2c1_unlocked_sources_diag_20260920.json"
OUTPUT_DIR = BACKEND_ROOT / "scripts" / "output"
BACKUP_DIR = BACKEND_ROOT.parent / "backups"

EXPECTED_EXEC_HASH = "1a0ed7f03bbc84a6920476e47fd5684d4f1b4f3ecdf4207a87569a1100887cda"
EXPECTED_DIAG_HASH = "096a26594c755cad08abc24f438c611d127b35394503976361827c116bcdc885"
...
VERIFY_DB = "digitaliza_sandbox_backup_verify_phase2c1"
...
def main() -> None:
    ...
    if diag_hash != EXPECTED_DIAG_HASH:
    ...
    backup_path = BACKUP_DIR / f"digitaliza_sandbox_pre_cleanup_phase2c1_{ts}.sql"
    dump_cmd = [
    ...
    with engine.connect() as conn:
        ...
        try:
            apply_report = apply_phase2c1_sources_cleanup(
                conn,
                execution_manifest,
                protected_manifest,
                STRUCTURED_PATH,
            ...
            out = OUTPUT_DIR / f"cleanup_phase2c1_sources_apply_{ts}.json"
        ...
        json.dumps(
            {
                ...
                "counts_after": {
                    ...
                    for k in ("actuaciones", "denuncia", "orden_trabajo", "inspeccion")
                },
                "users_fk_free": apply_report["users_unlock"]["users_test_fk_free_after_2c1"],
                "report": str(out),
            },
            indent=2,
        )

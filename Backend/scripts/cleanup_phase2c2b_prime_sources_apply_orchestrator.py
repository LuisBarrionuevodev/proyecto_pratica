#!/usr/bin/env python
"""
PREDEPLOY-CLEANUP.3H.4 — backup + apply FASE 2C.2B' (38 act + 38 OT).
"""
...
from app.domains.predeploy_cleanup.apply_phase2c2b_prime_service import (
    EXPECTED_APPLY_2C2A_PRIME_HASH,
    EXPECTED_EXEC_HASH,
    EXPECTED_PROT_HASH,
    EXPECTED_SOURCE_DIAG_3H_HASH,
    apply_phase2c2b_prime_sources_cleanup,
    preflight_phase2c2b_prime_apply,
)
...
VERIFY_DB = "digitaliza_sandbox_backup_verify_phase2c2b_prime"
...
def main() -> None:
    ...
    if not EXEC_PATH.is_file():
        raise SystemExit(f"Archivo requerido no encontrado: {EXEC_PATH}")
    if not PROTECTED_PATH.is_file():
        raise SystemExit(f"Archivo requerido no encontrado: {PROTECTED_PATH}")
    ...
    if execution_manifest.get("source_apply_2c2a_prime_sha256") != EXPECTED_APPLY_2C2A_PRIME_HASH:
        raise SystemExit("ABORT: source_apply_2c2a_prime_sha256 mismatch")
    if execution_manifest.get("source_diag_3h_sha256") != EXPECTED_SOURCE_DIAG_3H_HASH:
        raise SystemExit("ABORT: source_diag_3h_sha256 mismatch")
    if prot_hash != EXPECTED_PROT_HASH:
        raise SystemExit(f"ABORT: protected hash {prot_hash}")

    report["manifest_hashes"] = {
        "execution": exec_hash,
        "source_apply_2c2a_prime": EXPECTED_APPLY_2C2A_PRIME_HASH,
        "source_diag_3h": EXPECTED_SOURCE_DIAG_3H_HASH,
        "protected": prot_hash,
    }
    ...
    backup_path = BACKUP_DIR / f"digitaliza_sandbox_pre_cleanup_phase2c2b_prime_{ts}.sql"
    dump_cmd = [
    ...
    with engine.connect() as conn:
        ...
        try:
            apply_report = apply_phase2c2b_prime_sources_cleanup(
                conn,
                execution_manifest,
                protected_manifest,
                confirm_database=DB_NAME,
            ...
            out = OUTPUT_DIR / f"cleanup_phase2c2b_prime_sources_apply_{ts}.json"
        ...
        json.dumps(
            {
                ...
                "counts_after": {
                    ...
                    for k in ("actuaciones", "orden_trabajo", "iniciador_ruta", "ruta_item")
                },
                "new_orphans": apply_report["new_orphans_post_apply"],
                "users_fk_free": apply_report["users_unlock"]["users_test_fk_free_after_2c2b_prime"],
                "report": str(out),
            },
            indent=2,
        )

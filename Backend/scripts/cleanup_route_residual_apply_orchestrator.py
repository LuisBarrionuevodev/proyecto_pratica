#!/usr/bin/env python
"""
PREDEPLOY-CLEANUP.3J.2 — backup + apply ROUTE-RESIDUAL (12 ruta_trabajo).
"""
...
from app.domains.predeploy_cleanup.apply_route_residual_service import (
    EXPECTED_EXEC_HASH,
    EXPECTED_PROT_HASH,
    EXPECTED_SOURCE_DIAG_HASH,
    apply_route_residual_cleanup,
    preflight_route_residual_apply,
)
...
EXEC_PATH = BACKEND_ROOT / "scripts" / "output" / "cleanup_execution_manifest_route_residual_20260920.json"
PRIME_MANIFEST = BACKEND_ROOT / "scripts" / "output" / "cleanup_execution_manifest_phase2c2a_prime_wrappers_20260920.json"
...
VERIFY_DB = "digitaliza_sandbox_backup_verify_route_residual"
...
def main() -> None:
    ...
    for p in (EXEC_PATH, PRIME_MANIFEST, PROTECTED_PATH):
        if not p.is_file():
    ...
    backup_path = BACKUP_DIR / f"digitaliza_sandbox_pre_cleanup_route_residual_{ts}.sql"
    dump_cmd = [
    ...
    with engine.connect() as conn:
        ...
        try:
            apply_report = apply_route_residual_cleanup(
                conn,
                execution_manifest,
                protected_manifest,
                prime_manifest_path=PRIME_MANIFEST,
            ...
            out = OUTPUT_DIR / f"cleanup_route_residual_apply_{ts}.json"
        ...
        json.dumps(
            {
                ...
                "counts_after": {
                    "ruta_trabajo": apply_report["counts_after"]["ruta_trabajo"],
                    "ruta_grupo": apply_report["counts_after"]["ruta_grupo"],
                },
                "users_fk_free": apply_report["users_unlock"]["users_test_fk_free_after_route_cleanup"],
                "report": str(out),
            },
            indent=2,
        )

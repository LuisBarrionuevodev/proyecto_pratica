#!/usr/bin/env python
"""
PREDEPLOY-CLEANUP.3I.2 — backup + apply FASE 2C.2C (36 notif + 20 comp).
"""
...
from app.domains.predeploy_cleanup.apply_phase2c2c_service import (
    EXPECTED_EXEC_HASH,
    EXPECTED_PROT_HASH,
    EXPECTED_SOURCE_DIAG_HASH,
    apply_phase2c2c_orphan_documents_cleanup,
    preflight_phase2c2c_apply,
)
...
EXEC_PATH = (
    ...
    / "scripts"
    / "output"
    / "cleanup_execution_manifest_phase2c2c_orphan_documents_20260920.json"
)
...
VERIFY_DB = "digitaliza_sandbox_backup_verify_phase2c2c"
...
def main() -> None:
    ...
    report["manifest_hashes"] = {
        "execution": exec_hash,
        "source_diag_3i": EXPECTED_SOURCE_DIAG_HASH,
        "protected": prot_hash,
    }
    ...
    backup_path = BACKUP_DIR / f"digitaliza_sandbox_pre_cleanup_phase2c2c_{ts}.sql"
    dump_cmd = [
    ...
    with engine.connect() as conn:
        ...
        try:
            apply_report = apply_phase2c2c_orphan_documents_cleanup(
                conn,
                execution_manifest,
                protected_manifest,
                confirm_database=DB_NAME,
            ...
            out = OUTPUT_DIR / f"cleanup_phase2c2c_orphan_documents_apply_{ts}.json"
        ...
        json.dumps(
            {
                ...
                "counts_after": {
                    ...
                    for k in ("notificacion", "comprobacion", "actuaciones", "orden_trabajo")
                },
                "users_fk_free": apply_report["users_unlock"]["users_test_fk_free_after_2c2c"],
                "report": str(out),
            },
            indent=2,
        )

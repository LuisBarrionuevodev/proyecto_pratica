...
def test_create_con_estados_bien_observado(app_ctx, items_catalogo):
...
def test_legacy_put_preserva_estados(app_ctx, items_catalogo):
    id_bano = _item_id(items_catalogo, "TIENE_BANO")
    id_deposito = _item_id(items_catalogo, "TIENE_DEPOSITO")
    act = crear_actuacion_desde_payload(
        _base_create_payload(
...
def test_explicit_clear_items(app_ctx, items_catalogo):
    id_bano = _item_id(items_catalogo, "TIENE_BANO")
    act = crear_actuacion_desde_payload(
...
def test_cambio_estado_bien_a_observado(app_ctx, items_catalogo):
    id_bano = _item_id(items_catalogo, "TIENE_BANO")
    act = crear_actuacion_desde_payload(
        _base_create_payload(items_acta_inspeccion=[{"item_id": id_bano, "estado": "BIEN"}])
    )
...
def test_replace_items(app_ctx, items_catalogo):
    id_bano = _item_id(items_catalogo, "TIENE_BANO")
    id_deposito = _item_id(items_catalogo, "TIENE_DEPOSITO")
...
def test_personas_sin_carnet_preserve_cero_valor(app_ctx, items_catalogo):
...
def test_personas_sin_carnet_null_rechazado(app_ctx, items_catalogo):
...
def test_personas_sin_carnet_sin_notificacion_falla(app_ctx, items_catalogo):
    act = crear_actuacion_desde_payload(_base_create_payload())
...
def test_checklist_sin_inspeccion_falla(app_ctx, items_catalogo):
    act = crear_actuacion_desde_payload(
        {
            "orden_trabajo_numero": _unique_ot(),
            "fecha_actuacion": "2026-09-09",
...
def test_create_legacy_sin_checklist(app_ctx, items_catalogo):
    act = crear_actuacion_desde_payload(_base_create_payload())
    ins = Inspeccion.query.filter_by(actuacion_id=act.id).first()
    assert ins is not None
    assert _count_junction(int(ins.id)) == 0
...
def test_quitar_acta_elimina_junctions(app_ctx, items_catalogo):
    id_bano = _item_id(items_catalogo, "TIENE_BANO")
    act = crear_actuacion_desde_payload(
        _base_create_payload(items_acta_inspeccion=[{"item_id": id_bano, "estado": "BIEN"}])
    )
...
def test_quitar_notificacion_elimina_personas_sin_carnet(app_ctx, items_catalogo):
    motivo = _resolve_motivo_nombre()
    act = crear_actuacion_desde_payload(
        _base_create_payload(
            notificacion={"acta_num": _unique_acta(), "motivos": [motivo]},
...
def test_create_si_no_true_y_false(app_ctx, items_catalogo):
...
def test_rechaza_si_no_con_estado(app_ctx, items_catalogo):
    id_hab = _item_id(items_catalogo, "TIENE_HABILITACION")
    act = crear_actuacion_desde_payload(_base_create_payload())
...
def test_rechaza_estado_con_valor_si_no(app_ctx, items_catalogo):
    id_bano = _item_id(items_catalogo, "TIENE_BANO")
    act = crear_actuacion_desde_payload(_base_create_payload())
    with pytest.raises(ValueError, match="valor_si_no"):
        aplicar_inspeccion_checklist_desde_payload(
...
def test_rechaza_estado_item_con_valor_si_no(app_ctx, items_catalogo):
    id_bano = _item_id(items_catalogo, "TIENE_BANO")
    act = crear_actuacion_desde_payload(_base_create_payload())
...
def test_catalogo_incluye_tipo_respuesta(app_ctx, items_catalogo):
...
def test_read_dto_tipado(app_ctx, items_catalogo):
    id_bano = _item_id(items_catalogo, "TIENE_BANO")
    id_hab = _item_id(items_catalogo, "TIENE_HABILITACION")
    act = crear_actuacion_desde_payload(
        _base_create_payload(

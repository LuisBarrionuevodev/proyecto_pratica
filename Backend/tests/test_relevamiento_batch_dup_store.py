"""Duplicados de establecimiento en lote relevamientos (InMemoryBatchStore), PR7.5/PR7.6/PR9.1."""
...
def test_relevamiento_misma_ubicacion_esquina_varias_filas_permitido():
    ...
    loc = "MAIPU|Y SALTA"
    est_a = build_relevamiento_establishment_key(
        "Maipu", "y Salta", mes=_MES, anio=_ANIO, rubro_id=1, angulo_esquina="NE", es_esquina=True
    )
    est_b = build_relevamiento_establishment_key(
        "Maipu", "y Salta", mes=_MES, anio=_ANIO, rubro_id=2, angulo_esquina="NE", es_esquina=True
    )
...
def test_relevamiento_misma_ubicacion_numero_distinto_rubro_permitido():
    ...
    batch_id = store.start_batch(kind="relevamientos")
    loc = "SAN MARTIN|234"
    est_a = build_relevamiento_establishment_key("San Martin", "234", mes=_MES, anio=_ANIO, rubro_id=1, es_esquina=False)
    est_b = build_relevamiento_establishment_key("San Martin", "234", mes=_MES, anio=_ANIO, rubro_id=2, es_esquina=False)
...
def test_relevamiento_misma_ubicacion_numero_mismo_establecimiento_duplicado():
    ...
    batch_id = store.start_batch(kind="relevamientos")
    loc = "SAN MARTIN|1009"
    est = build_relevamiento_establishment_key("San Martin", "1009", mes=_MES, anio=_ANIO, rubro_id=1, es_esquina=False)
...
def test_relevamiento_mismo_establecimiento_distinto_mes_permitido():
    store = InMemoryBatchStore()
    batch_id = store.start_batch(kind="relevamientos")
    loc = "SAN MARTIN|1009"
    est_mar = build_relevamiento_establishment_key("San Martin", "1009", mes=3, anio=_ANIO, rubro_id=1, es_esquina=False)
    est_may = build_relevamiento_establishment_key("San Martin", "1009", mes=5, anio=_ANIO, rubro_id=1, es_esquina=False)
    assert store.upsert_relevamiento_ubicacion(batch_id, "row-a", loc, False, est_mar) is None
    assert store.upsert_relevamiento_ubicacion(batch_id, "row-b", loc, False, est_may) is None
...
def test_clear_row_key_libera_clave_numero_para_otra_fila():
    ...
    batch_id = store.start_batch(kind="relevamientos")
    loc = "MITRE|500"
    est = build_relevamiento_establishment_key("Mitre", "500", mes=_MES, anio=_ANIO, rubro_id=1, es_esquina=False)
...
def test_clear_row_key_relev_quita_indice():
    ...
    batch_id = store.start_batch(kind="relevamientos")
    loc = "A|1"
    est = build_relevamiento_establishment_key("A", "1", mes=_MES, anio=_ANIO, rubro_id=1, es_esquina=False)
...
def test_misma_fila_cambia_de_numero_a_esquina_libera_indice():
    ...
    batch_id = store.start_batch(kind="relevamientos")
    loc = "SAN MARTIN|1009"
    est_num = build_relevamiento_establishment_key("San Martin", "1009", mes=_MES, anio=_ANIO, rubro_id=1, es_esquina=False)
    est_esq = build_relevamiento_establishment_key(
        "San Martin", "1009", mes=_MES, anio=_ANIO, rubro_id=1, angulo_esquina="NE", es_esquina=True
    )

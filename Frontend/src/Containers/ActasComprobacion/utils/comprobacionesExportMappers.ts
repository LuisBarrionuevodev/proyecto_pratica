...
export function mapOficioPendienteRow(row: IPendientesOficioItem): ComprobacionExportRow {
  return baseRow("oficio", {
    ...
    rubro: cellStr(row.rubro_nombre),
    comprobacionMotivo: cellStr(row.comprobacion_motivo),
    expedienteEnvioNumero: row.sin_expediente_envio ? "—" : cellStr(row.expediente_original_numero),
    expedienteEnvioAnio: row.sin_expediente_envio ? "" : cellStr(row.expediente_original_anio),
    fechaExpedienteEnvio: row.sin_expediente_envio ? "" : cellStr(row.expediente_original_fecha),
    oficioNumero: "",
    oficioAnio: "",
    fechaOficio: "",
    causa: "",
    ...
    estadoRecorrido: row.sin_expediente_envio ? "Sin expediente de envío" : "Pendiente oficio",
    reinspeccionEstado: "",
    inspectores: inspectoresFromParts(row),
    ...EMPTY_OFICIOS_EXP,
  });
...
export function mapReinspeccionPendienteRow(row: IReinspeccionOficioPendienteRow): ComprobacionExportRow {
  return baseRow("reinspeccion", {
    ...
    rubro: cellStr(row.rubro_nombre),
    comprobacionMotivo: cellStr(row.comprobacion_motivo),
    expedienteEnvioNumero: cellStr(
      row.expediente_envio_label ?? row.expediente_envio_numero ?? row.expediente_numero
    ),
    expedienteEnvioAnio: row.expediente_envio_label ? "" : cellStr(row.expediente_envio_anio ?? row.expediente_anio),
    fechaExpedienteEnvio: row.expediente_envio_label ? "" : cellStr(row.fecha_expediente_envio),
    oficioNumero: cellStr(row.oficio_numero),
    oficioAnio: row.oficio_anio != null ? String(row.oficio_anio) : "",

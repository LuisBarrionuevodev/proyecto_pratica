...
/**
 * Icono pin para cierres operativos (`ruta_realizado` verde, `ruta_no_realizado` ámbar).
 */
export function createOperativoPointIcon(properties: Record<string, unknown>): L.DivIcon {
  const layer = String(properties.map_layer ?? "");
  const estado = String(properties.estado_ejecucion ?? "");
  const fill =
    layer === "ruta_no_realizado" || estado === "NO_REALIZADO" ? COLORS.warning : COLORS.success;

...
describe("GestionNotificacion OPER-RUTA.5", () => {
  ...
  it("Pendiente reinspección integra OperRutaPoolAccionesCell", () => {
    ...
    expect(cell).toContain("Agregar a ruta");
  });
  ...
  it("pendiente muestra Agregar al pool", () => {
    expect(puedeAgregarAlPool({ iniciador_id: 1, estado_operativo_pool: "pendiente" })).toBe(true);
  });

  it("en_pool no muestra Agregar al pool", () => {
    expect(puedeAgregarAlPool({ iniciador_id: 1, estado_operativo_pool: "en_pool" })).toBe(false);
  });

  it("en_ruta_borrador no muestra acciones inválidas", () => {
    expect(puedeAgregarAlPool({ iniciador_id: 1, estado_operativo_pool: "en_ruta_borrador" })).toBe(false);
    expect(puedeAgregarARuta({ iniciador_id: 1, estado_operativo_pool: "en_ruta_borrador" })).toBe(false);
  });
});
